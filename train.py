...
def validate(model, loader, device, ablate=False):
    ...
    with torch.no_grad():
        for slices, masks in loader:
            ...
            total_loss += loss.item()
            total_dice += dice_score(out_masks, masks).item()
            del slices, masks, out_masks
            gc.collect()
    return total_loss / len(loader), total_dice / len(loader)

def train_model(cfg, feedback_type, train_loader, val_loader, device, seed):
    ...
    history = {'train_loss': [], 'val_loss': [], 'val_dice': [], 'cereb_loss': []}
    best_dice = 0.0
    ...
    for epoch in range(cfg.NUM_EPOCHS):
        ...
        print(f"Ep {epoch+1:02d} | {feedback_type:10} | Train Loss: {train_loss:.4f} | "
              f"Val Loss: {val_loss:.4f} | Cereb Loss: {cereb_loss:.4f} | Val Dice: {val_dice:.4f}")
